...
run: ## Run the MCP server
	uv run python -m mcp_finnhub.server

run-stdio: ## Run in stdio mode (for Claude desktop)
	uv run fastmcp run src/mcp_finnhub/server.py

run-http: ## Run HTTP server with uvicorn
	uv run uvicorn mcp_finnhub.server:app --host 0.0.0.0 --port 8000
...
all: clean install format lint test ## Full workflow
...
bump: ## Bump version across all files (usage: make bump VERSION=0.2.0)
	...
	@sed -i '' 's/^__version__ = ".*"/__version__ = "$(VERSION)"/' src/mcp_finnhub/__init__.py
	@echo "Updated:"
	@echo "  manifest.json:              $$(jq -r .version manifest.json)"
	@echo "  pyproject.toml:             $$(grep '^version' pyproject.toml)"
	@echo "  src/mcp_finnhub/__init__.py: $$(grep '__version__' src/mcp_finnhub/__init__.py)"
